...
export function getNewSynergyArray(newVsSynergyValueArray: ISynergy[], newWithSynergyValueArray: ISynergy[], oldVsSynergyValueArray: ISynergy[], oldWithSynergyValueArray: ISynergy[], isHeroRemoved?: boolean | undefined) {
    ...
    while (count < newVsSynergyValueArray.length || count < newWithSynergyValueArray.length) {
        ...
        if (vsArrayElement) {
            const oldSynergy = getHeroFromSynergyList(oldVsSynergyValueArray, vsArrayElement.heroId2)?.synergy ?? 0
            const newSynergy = getSynergyValue(oldSynergy, vsArrayElement.synergy * -1)
            const obj: ISynergy = {
        ...
        if (withArrayElement) {
            const oldSynergy = getHeroFromSynergyList(oldWithSynergyValueArray, withArrayElement.heroId2)?.synergy ?? 0
            const newSynergy = getSynergyValue(oldSynergy, withArrayElement.synergy)
            const obj: ISynergy = {
