...
import {getHeroBlockClass} from "./getHeroBlockClass";
import {IHeroes, IPickedHero} from "../../types/IHeroes";

describe("getHeroBlockClass", () => {
    const hero:IHeroes = {
        "id": 1,
    ...
    test('Герой пикнут', () => {
        expect(getHeroBlockClass(pickedHeroesPicked, hero, null)).toStrictEqual(['heroDisabled'])
    })
    test('Герой забанен', () => {
        expect(getHeroBlockClass(pickedHeroesBanned, hero, null)).toStrictEqual(['heroBanned'])
    })
    test('Стандартный кейс', () => {
        expect(getHeroBlockClass(pickedHeroesNot, hero, null)).toStrictEqual(['attribute__image-container'])
    })
    test('Поиск другого героя', () => {
        expect(getHeroBlockClass(pickedHeroesNot, hero, [2])).toStrictEqual(['attribute__image-container','not-searched'])
    })
})